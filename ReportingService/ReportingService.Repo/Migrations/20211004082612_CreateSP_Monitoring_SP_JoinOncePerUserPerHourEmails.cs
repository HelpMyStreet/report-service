...
{
    public partial class CreateSP_Monitoring_SP_JoinOncePerUserPerHourEmails : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql(@"
                CREATE PROCEDURE [Monitoring].[SP_JoinOncePerUserPerHourEmails]
	@Trigger VARCHAR(100)
  ...
  SELECT	[E].[RecipientUserID]
			,DATEADD(HH,DATEPART(HH,MIN([EventDate])), CAST(CAST(MIN([EventDate]) AS DATE) AS DATETIME)) AS [Date] 
			,[E].[MessageID]
  ...
  WHERE	[E].[RecipientUserID] IS NOT NULL
  ...
  CREATE TABLE [#ActualEmails_Grouped]
	( [RecipientUserID] INT
	  ...
	  PRIMARY KEY ([RecipientUserID],[Timestamp]) )
	...
	FROM	[Monitoring].[ExpectedEmails]
			...
			[#ActualEmails_Grouped]
			  ...
			  ON [EXP].[RecipientUserID] = [ACT].[RecipientUserID]
				   ...
				   AND DATEADD(HH,DATEPART(HH,[EXP].[TimestampExpected]), CAST(CAST([EXP].[TimestampExpected] AS DATE) AS DATETIME)) = [ACT].[Timestamp]
  ...
  INSERT INTO [Monitoring].[MonitoringTimestamps]
	VALUES (@Activity,@RunID,@Trigger,0,GETDATE(),@RowsAffected,@ProcessedTo)
        ...
        {
            migrationBuilder.Sql(@"
                DROP PROCEDURE [Monitoring].[SP_JoinOncePerUserPerHourEmails]
            ");
        }
    }
}
