...
{
    public partial class CreateSP_Monitoring_SP_JoinOncePerUserPerJobPerDayEmails : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql(@"
                CREATE PROCEDURE [Monitoring].[SP_JoinOncePerUserPerJobPerDayEmails]
	@Trigger VARCHAR(100)
  ...
  SELECT	@ProcessedTo = CASE	WHEN @ProcessedTo > MAX([TimestampExpected]) THEN MAX([TimestampExpected])
								ELSE @ProcessedTo
								END
  ...
  UPDATE	[ACT]
	...
	FROM	[Monitoring].[ExpectedEmails]
			...
			[#ActualEmails]
			  ...
			  ON [EXP].[RecipientUserID] = [ACT].[RecipientUserID]
				   AND [EXP].[JobID] = [ACT].[JobID]
				   AND CAST([EXP].[TimestampExpected] AS DATE) = CAST([MinTimestamp] AS DATE)
				   AND [EXP].[TemplateName] = @TemplateName
  ...
  INSERT INTO [Monitoring].[MonitoringTimestamps]
	VALUES (@Activity,@RunID,@Trigger,0,GETDATE(),@RowsAffected,@ProcessedTo)
        ...
        {
            migrationBuilder.Sql(@"
                DROP PROCEDURE [Monitoring].[SP_JoinOncePerUserPerJobPerDayEmails]
            ");
        }
    }
}
