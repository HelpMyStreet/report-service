...
{
	public partial class AddMonitoringLastActivity_Step2 : Migration
	{
		protected override void Up(MigrationBuilder migrationBuilder)
		{
			migrationBuilder.Sql(@"
ALTER PROCEDURE [Monitoring].[SP_MonthlyStats]
	@Date DATE
...
DELETE FROM [Monitoring].[LastActivity]

INSERT INTO [Monitoring].[LastActivity] (GroupId, GroupName,GroupCount,LastActiveDate,[% Ever Active],[% Active within last 90 days])
select  GroupId, GroupName, GroupCount, LastActiveDate, 
		round(([Ever Active]/GroupCount) * 100,2) as [% Ever Active],
		round(([Active within last 90 days]/GroupCount) * 100,2) as [% Active within last 90 days]
from
(
select	u.ReferringGroupId as GroupId, g.GroupName, 
		GroupCount,
		max(a.LastActiveDate) as LastActiveDate,
		sum(case when a.LastActiveDate is null then 0.0 else 1.0 end) as [Ever Active],
		sum(case when datediff(d,a.LastActiveDate,getdate()) <=90 then 1.0 else 0.0 end) as [Active within last 90 days]
from	[UserLatest].[User] u
		left outer join
		(
			select		max(DateRequested) as LastActiveDate, r.ReferringGroupId, j.VolunteerUserID
			from		RequestLatest.Job j inner join RequestLatest.Request r on j.RequestId = r.Id
			group by	r.ReferringGroupId, j.VolunteerUserID
		) a on u.ReferringGroupId = a.ReferringGroupId and u.id = a.VolunteerUserID
		inner join [GroupLatest].[Group] g on u.ReferringGroupId = g.ID
		inner join
		(
		select count(1) as GroupCount, ReferringGroupId
		from [UserLatest].[User] u
		group by ReferringGroupId
		) gc on u.ReferringGroupId = gc.ReferringGroupId
group by u.ReferringGroupId, g.GroupName,GroupCount
) a
